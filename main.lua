...
function love.load()
    jack = { 
        x=0,
        y=0,
        sprit= love.graphics.newImage("Sprites/spritesheet.png"),
        animation = {
            direction = "right",
            idle = true,
            frame = 1,
    ...
    for i=1, jack.animation.max_frame do 
...
function love.update(dt)
    ...
    if not jack.animation.idle then 
        ...
        if jack.animation.timer > 0.2 then 
            ...
            if jack.animation.frame > jack.animation.max_frame then 
                    jack.animation.frame = 1
            end
        end    
    end    
...
function love.draw()
    ...
    if jack.animation.direction == "right" then 
        love.graphics.draw(jack.sprit,quads[jack.animation.frame],jack.x,jack.y)
    else 
        love.graphics.draw(jack.sprit,quads[jack.animation.frame],jack.x,jack.y,0,-1,1,QUAD_WIDTH,0)
